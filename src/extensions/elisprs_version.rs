...
//! elisprs version segment — see `zshrs_version` for the shared
//! contract. Default binary is `elisprs`; output shape is `elisprs X.Y.Z`.
...
pub fn version(bin: &str, format: &str, ttl_secs: u64) -> Option<Vec<Value>> {
    ...
    Some(vec![json!({
        "contents": contents,
        "highlight_groups": ["elisprs_version", "elisprs", "information:regular"],
...
mod tests {
    ...
    fn version_missing_binary_returns_none() {
        let r = version("/nonexistent/elisprs-xyz", "{version}", 30);
        assert!(r.is_none());
    }
}
