...
//! vimlrs rkyv-cache segment — reports the on-disk allocation of the
//! authoritative compiled-bytecode archive at
//! `<root>/scripts.rkyv`. vimlrs (Rust VimL reimplementation) writes
//! its bytecode store as a single rkyv archive; this segment shows
//! "how warm is the vimlrs cache" at a glance.
//!
//! Pure filesystem probe — single `stat` of the archive file.
//!
//! Default resolution:
//! 1. `$VIMLRS_RKYV_CACHE`           — explicit override
//! 2. `$VIMLRS_HOME/scripts.rkyv`
//! 3. `$XDG_DATA_HOME/vimlrs/scripts.rkyv`
//! 4. `~/.vimlrs/scripts.rkyv`
//!
...
//! ```json
//! {
//!   "function": "powerliners.vimlrs.rkyv_cache",
//!   "args": {
//!     "path": "~/.vimlrs/scripts.rkyv",
//!     "format": "{icon} {size}",
...
fn default_path_with(
    ...
    if let Some(p) = get_env("VIMLRS_RKYV_CACHE") {
        return PathBuf::from(p);
    }
    if let Some(home) = get_env("VIMLRS_HOME") {
        return PathBuf::from(home).join("scripts.rkyv");
    }
    if let Some(xdg) = get_env("XDG_DATA_HOME") {
        let p = PathBuf::from(xdg).join("vimlrs").join("scripts.rkyv");
        if path_exists(&p) {
            return p;
        }
    }
    let home = get_env("HOME").unwrap_or_default();
    PathBuf::from(home).join(".vimlrs").join("scripts.rkyv")
}
...
pub fn rkyv_cache(path: &str, format: &str, show_when_empty: bool) -> Option<Vec<Value>> {
    ...
    Some(vec![json!({
        "contents": contents,
        "highlight_groups": ["vimlrs_rkyv_cache", "vimlrs", "information:regular"],
...
mod tests {
    ...
    fn tmpfile(name: &str, bytes: &[u8]) -> PathBuf {
        let p = std::env::temp_dir().join(format!("powerliners-vimlrs-{name}.rkyv"));
    ...
    fn stat_missing_returns_none() {
        assert!(stat_file(Path::new("/nonexistent/vimlrs-xyz.rkyv")).is_none());
    }
    ...
    fn rkyv_cache_missing_returns_none_when_hidden() {
        let r = rkyv_cache("/nonexistent/vimlrs-zzz.rkyv", "{size}", false);
        assert!(r.is_none());
    }

    #[test]
    fn rkyv_cache_missing_renders_zero_when_show_when_empty() {
        let r = rkyv_cache("/nonexistent/vimlrs-zzz.rkyv", "{size}", true).unwrap();
    ...
    #[test]
    fn default_path_vimlrs_rkyv_cache_wins_over_everything() {
        let g = env_map(&[
            ("VIMLRS_RKYV_CACHE", "/explicit/override.rkyv"),
            ("VIMLRS_HOME", "/vimlrs/home"),
            ("XDG_DATA_HOME", "/xdg"),
            ("HOME", "/home/u"),
        ]);
        assert_eq!(
    ...
    #[test]
    fn default_path_vimlrs_home_wins_over_xdg_and_home() {
        let g = env_map(&[
            ("VIMLRS_HOME", "/vimlrs/home"),
            ("XDG_DATA_HOME", "/xdg"),
            ("HOME", "/home/u"),
        ...
        assert_eq!(
            default_path_with(g, |_| true),
            PathBuf::from("/vimlrs/home/scripts.rkyv")
        );
    }
    ...
    fn default_path_uses_xdg_when_archive_exists() {
        ...
        assert_eq!(
            default_path_with(g, |_| true),
            PathBuf::from("/xdg/vimlrs/scripts.rkyv")
        );
    }
    ...
    fn default_path_skips_xdg_when_archive_missing() {
        ...
        assert_eq!(
            ...
            PathBuf::from("/home/u/.vimlrs/scripts.rkyv")
        );
    }

    #[test]
    fn default_path_falls_back_to_home_dot_vimlrs() {
    ...
    #[test]
    fn default_path_with_no_env_returns_relative_dot_vimlrs() {
        let g = env_map(&[]);
        assert_eq!(
            default_path_with(g, |_| false),
            PathBuf::from(".vimlrs/scripts.rkyv")
        );
    }
    ...
    #[test]
    fn rkyv_cache_highlight_group_chain_is_vimlrs_specific() {
        let p = tmpfile("hl2", &[0u8; 1]);
        let r = rkyv_cache(p.to_str().unwrap(), "{size}", false).unwrap();
        let groups = r[0]["highlight_groups"].as_array().unwrap();
        assert_eq!(groups[0].as_str().unwrap(), "vimlrs_rkyv_cache");
        assert_eq!(groups[1].as_str().unwrap(), "vimlrs");
    }
}
