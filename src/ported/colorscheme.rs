...
pub fn get_attrs_flag(attrs: &[String]) -> u32 {
    ...
    if attrs.iter().any(|a| a == "bold") {            // py:18  if 'bold' in attrs:
        attrs_flag |= ATTR_BOLD;                      // py:19
    }
    if attrs.iter().any(|a| a == "italic") {          // py:20
        attrs_flag |= ATTR_ITALIC;                    // py:21
    }
    if attrs.iter().any(|a| a == "underline") {       // py:22
...
impl Colorscheme {
    ...
    pub fn new(colorscheme_config: &Map<String, Value>, colors_config: &Map<String, Value>) -> Self {
        ...
        if let Some(colors_in) = colors_config.get("colors").and_then(|v| v.as_object()) {
            for (color_name, color) in colors_in {                  // py:45
                let pair = match color {
                    ...
                    Value::Array(a) if a.len() == 2 => {
                        ...
                        let hex = u64::from_str_radix(hex_str.trim_start_matches("0x"), 16)
                            .unwrap_or(0);
        ...
        if let Some(gradients_in) = colors_config.get("gradients").and_then(|v| v.as_object()) {
            for (gradient_name, gradient) in gradients_in {        // py:54
                let arr = gradient.as_array().cloned().unwrap_or_default();
                let entry: Value = if arr.len() == 2 {              // py:55
        ...
        Colorscheme { colors, gradients, groups, translations }
    }
    ...
    pub fn get_gradient(&self, gradient: &str, gradient_level: f64) -> Value {
        if let Some(g) = self.gradients.get(gradient) {           // py:63
    ...
    pub fn get_group_props(
        ...
        match group {
            Value::String(group_name) => {                         // py:69  isinstance(group, str)
                ...
                } else {                                           // py:76  return None
                    None
                }
            }
            _ => {                                                  // py:81
                if translate_colors {                              // py:82
    ...
    pub fn get_highlighting(
        ...
        let group_props = group_props.ok_or_else(|| {              // py:104-105
            format!("Highlighting groups not found in colorscheme: {}", groups.join(", "))
        })?;
...
pub const cterm_to_hex: [u64; 256] = [
    ...
    0x000000, 0xc00000, 0x008000, 0x804000, 0x0000c0, 0xc000c0, 0x008080, 0xc0c0c0, 0x808080, 0xff6060,
    // py:122  row 1
    0x00ff00, 0xffff00, 0x8080ff, 0xff40ff, 0x00ffff, 0xffffff, 0x000000, 0x00005f, 0x000087, 0x0000af,
    // py:123  row 2
    ...
    // py:138  row 17
    0xd75fd7, 0xd75fff, 0xd78700, 0xd7875f, 0xd78787, 0xd787af, 0xd787d7, 0xd787ff, 0xd7af00, 0xd7af5f,
    // py:139  row 18
    0xd7af87, 0xd7afaf, 0xd7afd7, 0xd7afff, 0xd7d700, 0xd7d75f, 0xd7d787, 0xd7d7af, 0xd7d7d7, 0xd7d7ff,
    // py:140  row 19
    0xd7ff00, 0xd7ff5f, 0xd7ff87, 0xd7ffaf, 0xd7ffd7, 0xd7ffff, 0xff0000, 0xff005f, 0xff0087, 0xff00af,
    // py:141  row 20
    0xff00d7, 0xff00ff, 0xff5f00, 0xff5f5f, 0xff5f87, 0xff5faf, 0xff5fd7, 0xff5fff, 0xff8700, 0xff875f,
    // py:142  row 21
    0xff8787, 0xff87af, 0xff87d7, 0xff87ff, 0xffaf00, 0xffaf5f, 0xffaf87, 0xffafaf, 0xffafd7, 0xffafff,
    // py:143  row 22
    0xffd700, 0xffd75f, 0xffd787, 0xffd7af, 0xffd7d7, 0xffd7ff, 0xffff00, 0xffff5f, 0xffff87, 0xffffaf,
    // py:144  row 23 — start of greyscale ramp (24 entries)
    0xffffd7, 0xffffff, 0x080808, 0x121212, 0x1c1c1c, 0x262626, 0x303030, 0x3a3a3a, 0x444444, 0x4e4e4e,
    // py:145  row 24
    0x585858, 0x626262, 0x6c6c6c, 0x767676, 0x808080, 0x8a8a8a, 0x949494, 0x9e9e9e, 0xa8a8a8, 0xb2b2b2,
    // py:146  row 25 (final 6 entries)
...
mod tests {
    ...
    fn get_highlighting_resolves_group() {
        ...
        let cs = Colorscheme::new(&colorscheme_config, &colors_config);
        let hl = cs.get_highlighting(&["info".to_string()], None, None).unwrap();
